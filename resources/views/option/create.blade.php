...
@section('form')
    ...
    <form method="post" action="{{ route('option.store') }}">
        ...
        <div class="table-responsive">
            <table class="table table-bordered mb-5 mt-5">
                    ...
                    <td>
                            <div class="form-group @if ($errors->has('confirm')) has-error @endif">
                                <textarea name="confirm" id="confirm" rows="6"
                                          ...
                                          class="form-control">
                                    {{ old('confirm', (isset($option) ? $option->confirm : '')) }}
                                </textarea>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div class="form-group d-inline-block @if ($errors->has('price')) has-error @endif" style="display: inline-block; min-width: 150px;">
                            ...
                            @if(isset($tax_classes))
                                <select name="tax_class_id" id="tax_class_id" class="form-control">
                                    @foreach ($tax_classes as $tax_class)
                                        <option {{ old('tax_class_id') == $tax_class->id ? 'selected' : '' }}
                                                value="{{ $tax_class->id }}"> {{ $tax_class->name }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('tax_class_id')) <p class="help-block">{{ $errors->first('tax_class_id') }}</p> @endif
                            @else
            ...
            <div class="action-btn-wrapper text-center mb-5">
